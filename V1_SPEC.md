...
| 2026-08-27 | The host-capability channel could be ours or proposed to pi upstream; the voice extension could be shared with `pi-termux-android-voice` or vendored | Vendor the extension here — its transport is being replaced and it will grow phone-specific behaviour Termux has no use for, so sharing would be a fork in disguise. Build the channel here too, but constrained so it could become a pi proposal later: namespaced host-neutral capability names, no Android specifics on the wire, transport isolated behind one module. pi `ExtensionAPI` has no host surface today, which is the whole gap. |
| 2026-08-27 | Mobile input is the real constraint, not compute; pi in Termux works but is unpleasant to type into | Voice **input** needs no work — any Android dictation keyboard types into the prompt box as normal text, the same conclusion `pi-termux-android-voice` reached. Voice **output** and every other phone-native capability need a host-capability channel (reverse RPC, Layer 3 → Layer 1), designed in ARCHITECTURE.md. Offline recognition via Vosk is the intended eventual direction but is not v1. Speaking aloud defaults off. |
