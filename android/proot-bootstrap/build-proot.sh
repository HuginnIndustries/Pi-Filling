#!/usr/bin/env bash
#
# build-proot.sh — Cross-compile proot + talloc for Android.
#
# Vendored from theamericanmaker/Kai @ build-proot.sh
#   commit-at-vendor: see ./VENDORED.md
#
# Only output paths are changed from upstream (see VENDORED.md). The
# proot commit, talloc version, NDK requirements, and build flags are
# preserved verbatim so we get Kai's reproducible build behavior.
#
# Usage: ./build-proot.sh [--clean]
#
# Requirements:
#   - Android NDK r29.x (stable, F-Droid-compliant). Set ANDROID_NDK_HOME
#     or have it under $ANDROID_HOME/ndk/29.*.
#   - Python 3 (talloc's WAF build system).
#   - git, curl, make.
#   - Network access to github.com (proot source) and samba.org (talloc).
#
# Output: .so files in .out/<abi>/ next to this script.
...
for abi in $ABIS; do
    ...
    # Remove .comment sections for F-Droid reproducible builds (Kai issue #91).
