...
- 2026-08-27 — architecture — Architecture mapped across three product layers and one historical spike module. Dependency direction is acyclic with Protocol.kt and SandboxState.kt as the stable base. All five secondary outputs written. Coverage COMPLETE; four residual unknowns recorded as open questions, all requiring a runtime test or a maintainer ruling rather than further reading. — [closeout](closeouts/2026-08-27-architecture.md)
- 2026-08-27 — defect-scan — 13 static findings across six passes (0 critical, 2 high, 6 medium, 5 low), coverage PARTIAL. Both high findings are on the wrapper-death path and interact: a request can be orphaned by a process exit during registration, no timeout breaks the resulting hang, and the adjacent path crashes rather than failing the session. The Node wrapper is markedly more defensive than the Kotlin layer, tracking the fact that it has 25 tests and android/ has none. — [closeout](closeouts/2026-08-27-defect-scan.md)
- 2026-08-27 — amendment:wrapper-death-hardening — Plan step A applied: findings 3.1, 2.1, 2.2 and 1.1 fixed and the wrapper-death path verified on hardware with a discriminating test. The timeout half of the work is implemented but not runtime-verified, and is tracked as a new open question rather than claimed as done. — [closeout](closeouts/2026-08-27-amendment-wrapper-death-hardening.md)
